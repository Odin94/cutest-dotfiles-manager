...
1. **`cmd/cdm/apply.go`**: **`runApply`** reads the `--dry-run`/`-d` flag, calls **`runner.GetConfigRoot`** for repo root, then **`apply.Run(root, dryRun)`**.
...
1. **`cmd/cdm/diff.go`**: **`runDiff`** gets root via **`runner.GetConfigRoot`**, then **`diff.Run(root)`**.
...
1. **`cmd/cdm/status.go`**: **`runStatus`** gets root via **`runner.GetConfigRoot`**, then **`status.Run(root)`**.
...
1. **`cmd/cdm/init.go`**: **`runInit`** gets cwd with **`os.Getwd()`**, sets `repo := args[0]` if present, and calls **`initcmd.Run(cwd, repo)`**.
...
1. **`cmd/cdm/add.go`**: **`runAdd`** gets root via **`runner.GetConfigRoot`**, takes `args[0]` as source path and optional `args[1]` as target-path, then **`addcmd.Run(root, sourcePath, targetPath)`**.
...
1. **`cmd/cdm/watch.go`**: **`runWatch`** gets root via **`runner.GetConfigRoot`**, builds **`signal.NotifyContext(cmd.Context(), os.Interrupt)`** and defers **`stop()`**, defines **`applyFn := func() (*apply.ApplyResult, error) { return apply.Run(root, false) }`**, and calls **`watch.Run(ctx, root, applyFn)`**.
...
1. **`cmd/cdm/undo.go`**: **`runUndoConflictApply`** gets root via **`runner.GetConfigRoot`**, then **`undo.Run(root)`**.
...
Handled by Cobra and Fang: **`fang.Execute`** wires the root command so **`-h`**/ **`--help`** and the built-in **`help`** subcommand show usage. Fang adds a **`completion`** subcommand that generates shell completion scripts (bash/zsh/fish); no custom code in this repo beyond registering the root and subcommands in **`main.go`**.
